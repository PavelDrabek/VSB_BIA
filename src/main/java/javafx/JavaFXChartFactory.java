...
import javafx.controllers.keyboard.JavaFXCameraKeyController;
import javafx.controllers.mouse.JavaFXCameraMouseController;
import javafx.controllers.mouse.JavaFXMousePickingController;
...
public class JavaFXChartFactory extends AWTChartComponentFactory {
     ...
     * @return
     */
    public ImageView bindImageView(AWTChart chart, ImageView imageView) {
...
//        imageView.fitWidthProperty();
        ...
        if (image != null) {
            ...
            imageView.setImage(image);
             ...
             System.out.print("image is null at init");
        }
    ...
    public ImageView bindImageView(AWTChart chart) {
        ...
        return bindImageView(chart, imageView);
    }
    ...
    public void bind(final ImageView imageView, AWTChart chart) {
        if (!(chart.getCanvas().getRenderer() instanceof JavaFXRenderer3d)) {
            System.out.print("NOT BINDING IMAGE VIEW TO CHART AS NOT A JAVAFX RENDERER");
            return;
        }

        // Set listener on renderer to update imageView
        JavaFXRenderer3d renderer = (JavaFXRenderer3d) chart.getCanvas().getRenderer();
        renderer.addDisplayListener(new JavaFXRenderer3d.DisplayListener() {
            @Override
            public void onDisplay(Image image) {
                if (image != null) {
                    imageView.setImage(image);
                } else {
                    System.out.print("image is null while listening to renderer");
                }
            }
        });
    }
...
//                 System.out.println("resize ok");
    ...
    protected void resetTo(Chart chart, double width, double height) {
        if (chart.getCanvas() instanceof OffscreenCanvas) {
            ...
            canvas.initBuffer(canvas.getCapabilities(), (int) width - 200, (int) height);
